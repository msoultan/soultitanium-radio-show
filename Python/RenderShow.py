...
if blnUseCLArgs:
	...
	objCLArgs = objCLParser.parse_args(["--projectpath", "C:\\Users\\mike\\Documents\\REAPER Media\\Soul-Titanium\\show xx - DEV TEMPLATE\\Show",
...
RenderOutputPath = objCLArgs.renderpath
...
if os.path.exists(RenderOutputPath):
...
for strStation in lstStations:
	...
	if strStation != conProjectTemplate:
		...
		strStationRandomWAVPath = os.path.join(strScriptPath, conAudioFolderName, strStation, conRandomSpotsSrcFolder)
	...
	with open(strMasterProjectFilePath, "r") as fShowTemplate, \
		...
		for strLine in fShowTemplate:
			...
			#update the rendered file name
				...
				fOutFile.write(conRenderFilePrefix + "\"" + os.path.join(RenderOutputPath, "show-" + strShowNumber + "-" + strStation + strRenderExt) + "\"\n")
...
#							print( " snapped to beginning of spot")
						...
						if strStation != conProjectTemplate:
							...
							if conSpotRandomOption in strItemOptions:
								...
								strWavSrcPath = os.path.join(strScriptPath, "Audio", strStation, "Random spots",lstRandomWAVsFilenames[intFileNumber])
								...
								strWavSrcPath = os.path.join(strScriptPath, "Audio", strStation, "General", strStation + "-" + strItemName + ".wav")
...
#							NOT SURE WHY THIS IS HERE - MIGHT NOT BE NEEDED
							strWavSrcPath = os.path.join(strScriptPath, "Audio\\General", conSilentWAVFilename)
							strWavSrcFolder = str(pathlib.Path(strWavSrcPath).parent)
...
#	print(" %s project file saved" % strStation)
	...
	if strStation != conProjectTemplate:
		try:
			os.remove(os.path.join(RenderOutputPath, "show-" + strShowNumber + "-" + strStation + strRenderExt))
		except OSError:
			pass
	...
	if blnRenderOn:
		...
		#		print("output:",subprocess.CompletedProcess)
			...
			if os.path.exists(os.path.join(RenderOutputPath, "show-" + strShowNumber + "-" + strStation + strRenderExt)):
				#file was rendered, tag it if it's an mp3
				if blnRenderMP3:
					#apply ID3 tag to newly created MP3
					print(" tagging MP3")
					subprocess.call("\"" + os.path.join(strScriptPath, "id3.exe") + "\" -12 -v -t \"Soul-Titanium Radio Show " + strShowNumber + " - " + strDJName.title() + "\" -a \"Mike Soultanian\" -g \"House\" \"" + os.path.join(RenderOutputPath, "show-" + strShowNumber + "-" + strStation + strRenderExt) + "\"")
